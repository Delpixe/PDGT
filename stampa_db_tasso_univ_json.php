...
$sql = "SELECT * FROM `Tasso Universitario`";    //query che andremo ad eseguire
  ...
  if ($result = mysqli_use_result($connection)) {              //tramite questa funzione preleviamo l'ultimo risultato (della query) eseguito sul database $connection
    while ($rows = mysqli_fetch_row($result)) {           //tramite questa funzione analizziamo tutte le righe (una dopo l'altra) partendo dalla 1° fino all'ultima, fermandoci appena viene restituito 'false'
      $final_array[] = array(
                  "Anno" => "$rows[0]",
                  "Regione" => "$rows[1]",
                  "Tasso femmine" => "$rows[2]",
                  "Tasso maschi" => "$rows[3]",    //memorizziamo nell'array le info che ci interessano
                  "Tasso femmine e maschi" => "$rows[4]"
                );
    }
...
$elencoUnivJson = json_encode($final_array);       //codifichiamo l'array in json per trasferimento dati tramite richiesta HTTP
